 ...
 */
public class GenresGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all genres").get("/api/genres").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new genres")
                        .post("/api/genres")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"genres\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_genres_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created genres").get("${new_genres_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created genres").delete("${new_genres_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Genres entity").exec(scn);
