...
/**
 * Performance test for the MovieGenres entity.
 */
public class MovieGenresGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all movieGenres").get("/api/movie-genres").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new movieGenres")
                        .post("/api/movie-genres")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_movieGenres_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created movieGenres").get("${new_movieGenres_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created movieGenres").delete("${new_movieGenres_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MovieGenres entity").exec(scn);
