...
/**
 * Performance test for the MovieResource entity.
 */
public class MovieResourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all movieResources").get("/api/movie-resources").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new movieResource")
                        .post("/api/movie-resources")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody("{" + "\"part\": 0" + ", \"linkEmbed\": \"SAMPLE_TEXT\"" + ", \"linkM3u8\": \"SAMPLE_TEXT\"" + "}")
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_movieResource_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created movieResource").get("${new_movieResource_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created movieResource").delete("${new_movieResource_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MovieResource entity").exec(scn);
