 ...
 */
public class HistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all histories").get("/api/histories").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new history")
                        .post("/api/histories")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"part\": 0" + ", \"stopTime\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created history").get("${new_history_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created history").delete("${new_history_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the History entity").exec(scn);
