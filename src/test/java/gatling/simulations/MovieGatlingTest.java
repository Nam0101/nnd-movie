 ...
 */
public class MovieGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all movies").get("/api/movies").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new movie")
                        .post("/api/movies")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"name\": \"SAMPLE_TEXT\"" +
                                ", \"originName\": \"SAMPLE_TEXT\"" +
                                ", \"isCompleted\": null" +
                                ", \"slug\": \"SAMPLE_TEXT\"" +
                                ", \"episodeCurrent\": \"SAMPLE_TEXT\"" +
                                ", \"episodeTotal\": 0" +
                                ", \"quality\": \"SAMPLE_TEXT\"" +
                                ", \"year\": 0" +
                                ", \"trailerUrl\": \"SAMPLE_TEXT\"" +
                                ", \"time\": \"SAMPLE_TEXT\"" +
                                ", \"content\": \"SAMPLE_TEXT\"" +
                                ", \"isSingle\": null" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created movie").get("${new_movie_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created movie").delete("${new_movie_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Movie entity").exec(scn);
